...
namespace TypeMockExamples.NUnitTests
{
	...
	public class AssertSyntaxTests : AssertionHelper
	{
		...
		{
			const string phrase = "Hello World!";
			string[] greetings = { "Hello!", "Hi!", "Hola!" };
		...
		public void EqualIgnoringCaseTests()
		{
			...
			Assert.That(new[] { "Hello", "World" }, Is.EqualTo(new object[] { "HELLO", "WORLD" }).IgnoreCase);
			Assert.That(new[] { "HELLO", "Hello", "hello" }, Is.All.EqualTo("hello").IgnoreCase);
		            
			// Inherited syntax
			Expect(phrase, EqualTo("hello world!").IgnoreCase);
			//Only available using new syntax
			Expect(phrase, Not.EqualTo("goodbye world!").IgnoreCase);
			Expect(new[] { "Hello", "World" }, EqualTo(new object[] { "HELLO", "WORLD" }).IgnoreCase);
			Expect(new[] { "HELLO", "Hello", "hello" }, All.EqualTo("hello").IgnoreCase);
		}
		...
		public void EqualityTestsWithTolerance_MixedFloatAndDouble()
		{
			...
			Assert.That(2.20492f, Is.EqualTo(2.2f).Within(0.01d), "Single actual, Single expected, Double tolerance");
			Assert.That(2.20492f, Is.EqualTo(2.2d).Within(0.01d), "Single actual, Double expected, Double tolerance");
			Assert.That(2.20492f, Is.EqualTo(2.2d).Within(0.01f), "Single actual, Double expected, Single tolerance");
		}
			...
			public void EqualityTestsWithTolerance_MixingTypesGenerally()
		{
			...
			Assert.That(202d, Is.EqualTo(200d).Within(2), "Double actual, Double expected, int tolerance");
			Assert.That(4.87m, Is.EqualTo(5).Within(.25), "Decimal actual, int expected, Double tolerance");
			Assert.That(4.87m, Is.EqualTo(5ul).Within(1), "Decimal actual, ulong expected, int tolerance");
			Assert.That(487, Is.EqualTo(500).Within(25), "int actual, int expected, int tolerance");
			Assert.That(487u, Is.EqualTo(500).Within(25), "uint actual, int expected, int tolerance");
			Assert.That(487L, Is.EqualTo(500).Within(25), "long actual, int expected, int tolerance");
			Assert.That(487ul, Is.EqualTo(500).Within(25), "ulong actual, int expected, int tolerance");
		}
		...
		{
			object[] ints = { 1, 2, 3, 4 };
			object[] doubles = { 0.99, 2.1, 3.0, 4.05 };
			object[] strings = { "abc", "bad", "cab", "bad", "dad" };
		...
		{
			object[] mixed = { 1, 2, "3", null, "four", 100 };
			object[] strings = { "abc", "bad", "cab", "bad", "dad" };
		...
		{
			int[] ints1To5 = { 1, 2, 3, 4, 5 };
			int[] twoThrees = { 1, 2, 3, 3, 4, 5 };
			int[] twoFours = { 1, 2, 3, 4, 4, 5 };
			...
			// Inherited syntax
			Expect(new[] { 2, 1, 4, 3, 5 }, EquivalentTo(ints1To5));
			Expect(new[] { 2, 2, 4, 3, 5 }, Not.EquivalentTo(ints1To5));
			Expect(new[] { 2, 4, 3, 5 }, Not.EquivalentTo(ints1To5));
			Expect(new[] { 2, 2, 1, 1, 4, 3, 5 }, Not.EquivalentTo(ints1To5));
		}
		...
		public void SubsetTests()
		{
			...
			// Inherited syntax
			Expect(new[] { 1, 3, 5 }, SubsetOf(ints1To5));
			Expect(new[] { 1, 2, 3, 4, 5 }, SubsetOf(ints1To5));
			Expect(new[] { 2, 4, 6 }, Not.SubsetOf(ints1To5));
		}
